...
def main():
    ...
    for payment in payments:
        ...
        if GENERATE_MERGED_PDFS:
            merged_invoice_elements = create_invoice_pdf(None, data, merged_invoice_elements)
            merged_receipt_elements = create_receipt_pdf(None, data, merged_receipt_elements)

    if GENERATE_MERGED_PDFS:
        doc = SimpleDocTemplate("output/merged_invoices.pdf", pagesize=letter, topMargin=0.5*inch, bottomMargin=0.5*inch)
        doc.build(merged_invoice_elements)

        doc = SimpleDocTemplate("output/merged_receipts.pdf", pagesize=letter, topMargin=0.5*inch, bottomMargin=0.5*inch)
